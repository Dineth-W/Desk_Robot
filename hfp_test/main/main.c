...
void esp_bt_gap_cb(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *param)
{
    ...
    ESP_LOGI(BT_HF_TAG, "Discovered device: %s", bda_str);

    for (int i = 0; i < param->disc_res.num_prop; i++) {

        if (param->disc_res.prop[i].type == ESP_BT_GAP_DEV_PROP_EIR) {

            if (get_name_from_eir(param->disc_res.prop[i].val,
                                  peer_bdname,
                                  &peer_bdname_len)) {

                ESP_LOGI(BT_HF_TAG,
                         "Discovered device name: '%s'",
                         peer_bdname);

                ESP_LOGI(BT_HF_TAG,
                         "Target device name: '%s'",
                         remote_device_name);
                ...
                ESP_LOGI(BT_HF_TAG,
                         "Discovered device has no local name in EIR");
            }
        }
    }
