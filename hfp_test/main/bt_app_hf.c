 ...
 *
 * IMPORTANT:
 * The stock ESP-IDF HFP-HF example uses one ring buffer for both directions.
 * That is an intentional loopback demo: Bluetooth RX is written into the
 * buffer and the same data is returned to Bluetooth TX. It is NOT a
 * microphone/speaker implementation.
 *
 * Our robot has two independent directions:
 *   Bluetooth RX -> speaker
 *   microphone   -> Bluetooth TX
 *
 * hfp_audio.c owns those two independent PCM queues and the I2S hardware.
 */
...
static void bt_app_hf_client_audio_open(void)
{
    ...
    ESP_LOGI(BT_HF_TAG, "HFP hardware audio bridge started");
}
...
static void bt_app_hf_client_audio_close(void)
{
    hfp_audio_stop();
    ESP_LOGI(BT_HF_TAG,
             "HFP hardware audio bridge stopped: RX packets=%lu bytes=%lu, TX packets=%lu bytes=%lu",
...
{
    /*
     * Bluetooth asks us for PCM microphone data.
     * This callback MUST remain non-blocking.
     */
    ...
    if ((s_audio_tx_packets % 100) == 0) {
        ESP_LOGI(BT_HF_TAG,
                 "HFP AUDIO TX(mic): packets=%lu bytes=%lu last=%lu",
                 (unsigned long)s_audio_tx_packets,
                 (unsigned long)s_audio_tx_bytes,
                 (unsigned long)sz);
    }
...
{
    /*
     * Bluetooth gives us decoded narrow-band PCM on this configuration:
     * WBS/mSBC is disabled, so the controller handles CVSD and the HCI
     * callback receives the audio samples used by the HFP loopback example.
     *
     * Do not feed this data back to HFP. Send it to the speaker queue.
     */
    if (!hfp_audio_put_rx_pcm(buf, sz)) {
        ESP_LOGW(BT_HF_TAG, "HFP speaker queue full; RX audio dropped");
    }
    ...
    if ((s_audio_rx_packets % 100) == 0) {
        ESP_LOGI(BT_HF_TAG,
                 "HFP AUDIO RX(speaker): packets=%lu bytes=%lu last=%lu",
                 (unsigned long)s_audio_rx_packets,
                 (unsigned long)s_audio_rx_bytes,
                 (unsigned long)sz);
    }
...
void bt_app_hf_client_cb(esp_hf_client_cb_event_t event, esp_hf_client_cb_param_t *param)
{
    ...
    ESP_LOGI(BT_HF_TAG, "HFP SLC connected - PBAP disabled");
